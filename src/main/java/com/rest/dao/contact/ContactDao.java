...
public class ContactDao {
    ...
    public List<Contact> getAllContacts(Integer userId){
    	...
    	List<Contact> allContactRecords =  handle.createQuery(getAllContactsQuery)
    ...
    public Contact getContactById(Integer userId,Integer contactId){
    	...
    	Contact contactRecord = handle.createQuery(getContactByIdQuery)
    							.map(new ContactMapper())
    							.bind("uid", userId)
    							.bind("cid", contactId)
    							.first();
    ...
    public List<Contact> getContactByLastName(Integer userId,String lastName){
    	...
    	List<Contact> contactsWithGivenLastName = handle.createQuery(getContactByLastNameQuery)
    													.map(new ContactMapper())
    													.bind("uid", userId)
    ...
    public List<Contact> getContactByName(Integer userId,String firstName, String lastName){
    	...
    	List<Contact> contactsWithGivenName = handle.createQuery(getContactByNameQuery)
    							   					.map(new ContactMapper()).bind("uid", userId)
    							   					.bind("firstName", firstName)
    							   					.bind("lastName", lastName)
    							   					.list();
    ...
    public boolean updateContact(Integer userId,Integer contactId, Contact contact){
    	...
    	return handle.createStatement(updateContactQuery)
        			 .bind("uid", userId)
        			 .bind("cid",contactId)
    ...
    public boolean insertContact(Integer userId,Integer contactId, Contact contact){
        ...
        return handle.createStatement(insertContactQuery)
        			 .bind("uid", userId)
        			 .bind("cid",contactId)
        			 .bind("firstname",contact.getFirstName() )
        			 .bind("lastname",contact.getLastName())
